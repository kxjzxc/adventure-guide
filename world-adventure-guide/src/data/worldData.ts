...
// ============================================================
// Route 路线
// 为每个相邻地点生成一条简单的折线（使用起点+1~2 个中间控制点 + 终点）
// ============================================================
function makeRoute(
  id: string,
  from: Place,
...
export const ROUTES: Route[] = [
  makeRoute('r-sg-bk', S, B, [
    '穿越暹罗湾，感受海洋与陆地的交替',
    '途经马来半岛的丛林与橡胶园',
    '从花园都市过渡到寺庙之城',
  ]),
  makeRoute('r-bk-hn', B, H, [
    '沿湄公河逆流而上，进入东南亚腹地',
    '经过泰东北依善地区的田园',
    '从佛教之国进入越南的红色大地',
  ]),
  makeRoute('r-hn-km', H, K, [
    '翻越黄连山脉，告别红河三角洲',
    '穿过老街口岸，进入云贵高原',
    '海拔逐渐升高，天气越来越凉爽',
  ]),
  makeRoute('r-km-cd', K, C, [
    '横断山脉的壮丽山河：金沙江、大渡河',
    '从热带过渡到亚热带内陆盆地',
    '途经乐山大佛与峨眉山麓',
  ]),
  makeRoute('r-cd-xa', C, X, [
    '翻越秦岭，地理上的南北分界',
    '从巴蜀天府进入八百里秦川',
    '剑门蜀道、嘉陵江、汉中平原',
  ]),
  makeRoute('r-xa-bj', X, BJ, [
    '沿汾渭平原北上，穿过黄河',
    '途经平遥、太原，触摸晋商的故园',
    '太行山脉之后，便是燕山脚下的北京',
  ]),
...
export function getContentsByPlace(placeId: string): ContentItem[] {
  return CONTENTS.filter((c) => c.placeId === placeId);
}

/** 根据起点终点生成一条冒险的 routeIds 与 placeIds（按已有数据组合） */
export function buildAdventurePath(fromId: string, toId: string): {
  placeIds: string[];
  routeIds: string[];
} | null {
  const fromIdx = PLACES.findIndex((p) => p.id === fromId);
  const toIdx = PLACES.findIndex((p) => p.id === toId);
